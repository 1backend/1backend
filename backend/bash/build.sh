...
sudo docker rm $2_$3
echo "CREATE DATABASE IF NOT EXISTS $2_$3; CREATE USER IF NOT EXISTS $2_$3@'%'; GRANT ALL PRIVILEGES ON $2_$3.* TO $2_$3@'%' identified by '$4'; GRANT ALL PRIVILEGES ON $2_$3.* TO $2_$3@'localhost' identified by '$4'; FLUSH PRIVILEGES;" | mysql -h localhost -P 3306 --protocol=tcp -u root -proot
echo "SET PASSWORD FOR '$2_$3'@'%' = PASSWORD('$4');" | mysql -h localhost -P 3306 --protocol=tcp -u root -proot
