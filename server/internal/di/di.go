...
func BigBang(options *universe.Options) (*Universe, error) {
	...
	if err := loadContactAuthProviders(options); err != nil {
		return nil, err
	}
...
func loadContactAuthProviders(options *universe.Options) error {
	providers := map[string]universe.ContactAuthProviderConfig{}
	for providerID, provider := range options.ContactAuthProviders {
		mergeContactAuthProvider(providers, providerID, provider)
	}

	for _, envName := range []string{"OB_AUTH_PROVIDERS", "OB_CONTACT_AUTH_PROVIDERS"} {
		raw := os.Getenv(envName)
		if raw == "" {
			continue
		}
		if err := mergeContactAuthProvidersJSON(providers, raw); err != nil {
			return errors.Wrapf(err, "failed to parse %s", envName)
		}
	}

	if raw := os.Getenv("OB_OIDC_PROVIDERS"); raw != "" {
		if err := mergeOIDCProvidersJSON(providers, raw); err != nil {
			return errors.Wrap(err, "failed to parse OB_OIDC_PROVIDERS")
		}
	}

	mergeContactAuthProviderFromEnv(
		providers,
		"google",
		"OB_GOOGLE_OIDC_CLIENT_ID",
		"OB_GOOGLE_OIDC_CLIENT_SECRET",
		universe.ContactAuthProviderConfig{},
	)
	mergeContactAuthProviderFromEnv(
		providers,
		"facebook",
		...
		universe.ContactAuthProviderConfig{
			GraphVersion: os.Getenv("OB_FACEBOOK_GRAPH_VERSION"),
		},
	)
	mergeContactAuthProviderFromEnv(
		providers,
	...
	mergeContactAuthProviderFromEnv(
		...
		universe.ContactAuthProviderConfig{
			IssuerURL: os.Getenv("OB_GITLAB_OIDC_ISSUER_URL"),
		},
	)
	mergeContactAuthProviderFromEnv(
		providers,
	...
	if len(providers) == 0 {
		options.ContactAuthProviders = nil
		return nil
	}
	options.ContactAuthProviders = providers
	return nil
}
...
func mergeContactAuthProviderFromEnv(
	providers map[string]universe.ContactAuthProviderConfig,
	providerID string,
	clientIDEnv string,
	clientSecretEnv string,
	extra universe.ContactAuthProviderConfig,
) {
	provider := extra
	provider.Id = providerID
	provider.ClientID = os.Getenv(clientIDEnv)
	provider.ClientSecret = os.Getenv(clientSecretEnv)
	if provider.ClientID == "" &&
		provider.ClientSecret == "" &&
		provider.IssuerURL == "" &&
		provider.GraphVersion == "" {
		return
	}
	mergeContactAuthProvider(providers, providerID, provider)
}
