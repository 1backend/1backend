...
func (us *UserService) RegisterRoutes(router *mux.Router) {
	...
	router.HandleFunc("/user-svc/otp/request", appl(func(w http.ResponseWriter, r *http.Request) {
		us.RequestOTP(w, r)
	})).
		Methods("OPTIONS", "POST")

	router.HandleFunc("/user-svc/otp/verify", appl(func(w http.ResponseWriter, r *http.Request) {
		us.VerifyOTP(w, r)
	})).
		Methods("OPTIONS", "POST")
