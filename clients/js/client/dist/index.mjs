...
class UserSvcApi extends BaseAPI {
    ...
    /**
     * Check whether the caller user has a specific permission. Ideally, this endpoint should rarely be used, as the JWT token already includes all user roles. Caching the `List Permissions` and `List Permits` responses allows services to determine user authorization without repeatedly calling this endpoint.  This endpoint should have no other parameters apart from the caller and the permission so it can be cached easily.
     * Has Permission
     */
