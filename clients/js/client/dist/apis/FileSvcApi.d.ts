...
export declare class FileSvcApi extends runtime.BaseAPI {
     ...
     * List Downloads
     */
    listDownloadsRaw(requestParameters: ListDownloadsRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<FileSvcDownloadsResponse>>;
    /**
     * List download details.  Requires the `file-svc:download:view` permission.
     * List Downloads
     */
    listDownloads(requestParameters?: ListDownloadsRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<FileSvcDownloadsResponse>;
    /**
     ...
     */
    serveDownloadRaw(requestParameters: ServeDownloadRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<any>>;
    /**
     * Serves a previously downloaded file based on its URL.
     * Serve a Downloaded file
     */
    serveDownload(requestParameters: ServeDownloadRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<any>;
    /**
     ...
     */
    serveUploadRaw(requestParameters: ServeUploadRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<any>>;
    /**
     * Retrieves and serves a previously uploaded file using its File ID. Note: The `ID` and `FileID` fields of an upload are different. - `FileID` is a unique identifier for the file itself. - `ID` is a unique identifier for a specific replica of the file. Since 1Backend is a distributed system, files can be replicated across multiple nodes. This means each uploaded file may have multiple records with the same `FileID` but different `ID`s.
     * Serve an Uploaded File
     */
    serveUpload(requestParameters: ServeUploadRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<any>;
    /**
     ...
     * Upload a File
     */
    uploadFileRaw(initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<FileSvcUploadFileResponse>>;
    /**
     * Uploads a file to the server. Currently if using the clients only one file can be uploaded at a time due to this bug https://github.com/OpenAPITools/openapi-generator/issues/11341 Once that is fixed we should have an `PUT /file-svc/uploads`/uploadFiles (note the plural) endpoints. In reality the endpoint \"unofficially\" supports multiple files. YMMV.  Requires the `file-svc:upload:create` permission.
     * Upload a File
     */
    uploadFile(initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<FileSvcUploadFileResponse>;
}
