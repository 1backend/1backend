...
func instrumentDB(driverName, tableName, target string, db DB) DB {
	if db == nil {
		return nil
	}
	return &otelDB{
...
func (db *otelDB) Query(query string, args ...interface{}) (*sql.Rows, error) {
	...
	telemetry.RecordSQLStatementTarget(context.Background(), db.driverName, db.tableName, db.target, query, started, err)
	return res, err
}
...
func (db *otelDB) QueryRow(query string, args ...interface{}) *sql.Row {
	...
	telemetry.RecordSQLStatementTarget(context.Background(), db.driverName, db.tableName, db.target, query, started, nil)
	return row
}
...
func (tx *otelTx) Query(query string, args ...interface{}) (*sql.Rows, error) {
	...
	telemetry.RecordSQLStatementTarget(context.Background(), tx.driverName, tx.tableName, tx.target, query, started, err)
	return res, err
}
...
func (tx *otelTx) QueryRow(query string, args ...interface{}) *sql.Row {
	...
	telemetry.RecordSQLStatementTarget(context.Background(), tx.driverName, tx.tableName, tx.target, query, started, nil)
	return row
}
